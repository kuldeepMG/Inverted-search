...
int insert_hash(hash_t *hash_table, int index, char *word, char *filename)
{
    ...
    /*
     * CASE 1:
     * Hash index is NOT empty
     * Traverse main nodes to check if word already exists
     */
    while (mtemp)
    {
        /*
         * CASE 1.1:
         * Word already exists in main list
         */
        if (strcmp(mtemp->word, word) == 0)
        {
            ...
            /*
             * CASE 1.1.1:
             * Word exists AND file already exists
             */
            while (stemp)
            {
            ...
            /*
             * CASE 1.1.2:
             * Word exists BUT file is new
             * Insert new sub node at FIRST
             */
    ...
    /*
     * CASE 2.1:
     * First main node at this hash index
     */
    if (prev == NULL)
    {
    ...
    /*
     * CASE 2.2:
     * Insert main node at END of list
     */
    else
    {
...
//these are buffers to store temperorly
    int index;  
    char word[100];
    int file_count;
    ...
    while (fscanf(fp, "%d", &index) == 1)   // Read index
    {
        ...
        //assign from temp buffer to new main node structure
        strcpy(new_main->word, word);
        ...
        {
            hash_table[index].link = new_main;  //adding first main node to hash table
        }
        else
        {
            ...
            temp->m_link = new_main;    //continue adding of main node after first main node
        }
        ...
        while (1)
        {
            ...
            //store  from buffer to new sub node structure 
            strcpy(new_sub->filename, filename);
            ...
            {
                new_main->s_link = new_sub; //adding first sub node
            }
            else
            {
                ...
                while (stemp->link)
                    stemp = stemp->link;
                stemp->link = new_sub;  //continue adding sub node after first sub node
            }
        }
    }
